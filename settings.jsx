...
function SettingsView({ lang, setLang, theme, setTheme, density, setDensity, team: teamProp, onSaveTeamMember, onDeleteTeamMember, tags: tagsProp, onSaveTag, onDeleteTag }) {
  ...
  return (
    <div style={{ padding: "32px 40px 80px", overflow: "auto", flex: 1, display: "flex", flexDirection: "column", gap: 48, maxWidth: 900 }}>
      ...
      {window.CURRENT_USER?.email === "tomtautz@gmail.com" && (
        <TenantAdminSection lang={lang} />
      )}
...
/* ══════════════════════════════════════════════════════════════════
   TenantAdminSection — only rendered for tomtautz@gmail.com
══════════════════════════════════════════════════════════════════ */

// Create a Firebase Auth user via REST API — does NOT sign out current user
async function createAuthUser(email, password) {
  const res = await fetch(
    `https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${FB_CFG.apiKey}`,
    {
      method: "POST",
...
// Seed a brand-new tenant with minimal structure
async function seedNewTenant(tenantId, companyName) {
  ...
  // "Nicht zugeordnet" folders
  await col("folders").doc("f-unsorted").set({
    id: "f-unsorted", name: "Nicht zugeordnet",
    parent: null, count: 0, pinned: false, sortOrder: 9999,
    updated: new Date().toISOString().slice(0, 10),
    coverHues: [200, 220, 240, 260], owner: "system",
  });
  await col("pdfFolders").doc("p-unsorted").set({
    id: "p-unsorted", name: "Nicht zugeordnet",
    parent: null, count: 0, pinned: false, sortOrder: 9999,
    updated: new Date().toISOString().slice(0, 10),
    coverHues: [200, 220, 240, 260], owner: "system",
  });

  console.log(`✓ Tenant "${tenantId}" (${companyName}) angelegt`);
}

function TenantAdminSection({ lang }) {
  const [tenants, setTenants] = useStateS([]);
  const [loadingTenants, setLoadingTenants] = useStateS(true);

  // Tenant form
  const [tId,          setTId]          = useStateS("");
  const [tName,        setTName]        = useStateS("");
  const [tCreating,    setTCreating]    = useStateS(false);
  const [tMsg,         setTMsg]         = useStateS(null); // {ok, text}

  // User form
  const [uEmail,       setUEmail]       = useStateS("");
  const [uPassword,    setUPassword]    = useStateS("");
  const [uTenant,      setUTenant]      = useStateS("");
  const [uCreating,    setUCreating]    = useStateS(false);
  const [uMsg,         setUMsg]         = useStateS(null);
  ...
  async function loadTenants() {
    ...
    try {
      ...
      const list = await Promise.all(snap.docs.map(async d => {
        const settings = await window.db.doc(`tenants/${d.id}/settings/global`).get();
        const usersSnap = await window.db.collection("tenants").doc(d.id).collection("users").get();
        return {
          id: d.id,
          name: settings.data()?.companyName || d.id,
          userCount: usersSnap.size,
        };
      }));
      list.sort((a, b) => a.id.localeCompare(b.id));
      setTenants(list);
      if (!uTenant && list.length > 0) setUTenant(list[0].id);
    } catch (e) {
      console.warn("loadTenants:", e.message);
    } finally {
      setLoadingTenants(false);
    }
  }

  useEffectS(() => { loadTenants(); }, []);
  ...
  async function handleCreateTenant(e) {
    ...
    if (!id || !name) return;
    setTCreating(true); setTMsg(null);
    try {
      const exists = await window.db.doc(`tenants/${id}/settings/global`).get();
      if (exists.exists) throw new Error(`Tenant „${id}" existiert bereits.`);
      await seedNewTenant(id, name);
      setTMsg({ ok: true, text: `✓ Mandant „${id}" (${name}) angelegt` });
      setTId(""); setTName("");
      await loadTenants();
      setUTenant(id);
    } catch (e) {
      setTMsg({ ok: false, text: e.message });
    } finally {
      setTCreating(false);
    }
  }
  ...
  async function handleCreateUser(e) {
    ...
    const tenantId = uTenant;
    if (!email || !password || !tenantId) return;
    setUCreating(true); setUMsg(null);
    try {
      // Create Firebase Auth user (no sign-out of current user)
      const { uid } = await createAuthUser(email, password);

      // Store in Firestore: tenant users list
      await window.db.collection("tenants").doc(tenantId).collection("users").doc(uid).set({
        uid, email, role: "editor",
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      // Also store reverse lookup
      await window.db.collection("userTenants").doc(uid).set({ tenantId, email });

      setUMsg({ ok: true, text: `✓ Nutzer ${email} für Mandant „${tenantId}" angelegt` });
      setUEmail(""); setUPassword("");
      await loadTenants();
    } catch (e) {
      setUMsg({ ok: false, text: e.message });
    } finally {
      setUCreating(false);
    }
  }
  ...
  const inp = {
    height: 36, padding: "0 10px", border: "1px solid var(--line-strong)",
    borderRadius: 4, background: "var(--bg)", color: "var(--fg)",
    fontSize: 13, outline: "none", boxSizing: "border-box",
  };

  return (
    <section style={{ marginBottom: 56 }}>
      <SectionLabel>Super-Admin · Mandanten &amp; Nutzer</SectionLabel>

      {/* ── Tenant creation ── */}
      <div style={{ background: "var(--panel)", border: "1px solid var(--line-strong)", borderRadius: 6, marginBottom: 16, overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--line)" }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Neuen Mandanten anlegen</div>
          <div style={{ fontSize: 12, color: "var(--muted)" }}>Tenant-ID = Subdomain (z.B. <span style={{ fontFamily: "var(--font-mono)" }}>phaeno</span> → phaeno.picpop.de)</div>
        </div>
        <form onSubmit={handleCreateTenant} style={{ padding: "16px 20px", display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
          <div style={{ flex: "0 0 180px" }}>
            <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 5, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Tenant-ID</div>
            <input
              value={tId} onChange={e => setTId(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-"))}
              placeholder="z.B. phaeno" required
              style={{ ...inp, width: "100%" }}
            />
          </div>
          <div style={{ flex: "1 1 200px" }}>
            <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 5, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Firmenname</div>
            <input
              value={tName} onChange={e => setTName(e.target.value)}
              placeholder="z.B. Phæno Wolfsburg" required
              style={{ ...inp, width: "100%" }}
            />
          </div>
          <button type="submit" disabled={tCreating || !tId || !tName}
            className="btn primary" style={{ height: 36, flexShrink: 0 }}>
            {tCreating ? "Anlegen…" : "+ Mandant anlegen"}
          </button>
        </form>
        {tMsg && (
          <div style={{ margin: "0 20px 16px", fontSize: 12, padding: "8px 12px", borderRadius: 4, background: tMsg.ok ? "oklch(0.95 0.04 145)" : "var(--accent-soft)", color: tMsg.ok ? "oklch(0.4 0.14 145)" : "var(--accent)" }}>
            {tMsg.text}
          </div>
        )}
      </div>
      ...
      {/* ── Tenant list ── */}
      <div style={{ background: "var(--panel)", border: "1px solid var(--line-strong)", borderRadius: 6, marginBottom: 24, overflow: "hidden" }}>
        <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--line)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 12, fontWeight: 600, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--muted)" }}>
            Bestehende Mandanten ({tenants.length})
          </div>
          <button className="btn ghost sm" onClick={loadTenants} style={{ fontSize: 11 }}>↺ Aktualisieren</button>
        </div>
        {loadingTenants ? (
          <div style={{ padding: "20px", textAlign: "center", color: "var(--muted)", fontSize: 12 }}>Lädt…</div>
        ) : tenants.length === 0 ? (
          <div style={{ padding: "20px", textAlign: "center", color: "var(--muted)", fontSize: 12 }}>Noch keine Mandanten angelegt.</div>
        ) : (
          tenants.map((t, i) => (
            <div key={t.id} style={{ display: "flex", alignItems: "center", padding: "11px 20px", borderBottom: i < tenants.length - 1 ? "1px solid var(--line)" : "none", gap: 16 }}>
              <div style={{ width: 28, height: 28, borderRadius: 4, background: "var(--hover)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 700, color: "var(--muted)", flexShrink: 0 }}>
                {t.id[0].toUpperCase()}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 500 }}>{t.name}</div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--muted)" }}>{t.id}</div>
              </div>
              <div style={{ fontSize: 12, color: "var(--muted)", flexShrink: 0 }}>
                {t.userCount} {t.userCount === 1 ? "Nutzer" : "Nutzer"}
              </div>
              <a
                href={`?t=${t.id}`}
                style={{ fontSize: 11, color: "var(--accent)", fontFamily: "var(--font-mono)", flexShrink: 0 }}
              >
                ?t={t.id} ↗
              </a>
            </div>
      ...
      <div style={{ background: "var(--panel)", border: "1px solid var(--line-strong)", borderRadius: 6, overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--line)" }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Neuen Nutzer anlegen</div>
          <div style={{ fontSize: 12, color: "var(--muted)" }}>Firebase Auth Account erstellen und einem Mandanten zuweisen</div>
        </div>
        <form onSubmit={handleCreateUser} style={{ padding: "16px 20px", display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 200px" }}>
            <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 5, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.08em" }}>E-Mail</div>
            <input type="email" value={uEmail} onChange={e => setUEmail(e.target.value)}
              placeholder="nutzer@firma.de" required autoComplete="off"
              style={{ ...inp, width: "100%" }} />
          </div>
          <div style={{ flex: "0 0 190px" }}>
            <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 5, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Passwort</div>
            <div style={{ position: "relative" }}>
              <input type={showPw ? "text" : "password"} value={uPassword} onChange={e => setUPassword(e.target.value)}
                placeholder="mind. 6 Zeichen" required autoComplete="new-password"
          ...
          <div style={{ flex: "0 0 160px" }}>
            <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 5, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Mandant</div>
            <select value={uTenant} onChange={e => setUTenant(e.target.value)} required
          ...
          <button type="submit" disabled={uCreating || !uEmail || !uPassword || !uTenant}
            className="btn primary" style={{ height: 36, flexShrink: 0 }}>
            {uCreating ? "Anlegen…" : "+ Nutzer anlegen"}
          </button>
        </form>
        {uMsg && (
          <div style={{ margin: "0 20px 16px", fontSize: 12, padding: "8px 12px", borderRadius: 4, background: uMsg.ok ? "oklch(0.95 0.04 145)" : "var(--accent-soft)", color: uMsg.ok ? "oklch(0.4 0.14 145)" : "var(--accent)" }}>
            {uMsg.text}
          </div>
        )}
        {/* Quick login URL hint */}
        {uMsg?.ok && uTenant && (
          <div style={{ margin: "0 20px 16px", fontSize: 12, padding: "10px 12px", borderRadius: 4, background: "var(--hover)", color: "var(--fg-2)", lineHeight: 1.6 }}>
            <strong>Login-URL für diesen Nutzer:</strong><br/>
            Lokal: <span style={{ fontFamily: "var(--font-mono)", color: "var(--fg)" }}>localhost:8080?t={uTenant}</span><br/>
            Prod: <span style={{ fontFamily: "var(--font-mono)", color: "var(--fg)" }}>{uTenant}.picpop.de</span>
          </div>
        )}
      </div>
    </section>
